...
function themedHooks(tags: string[], persona?: string): Hook[] {
  ...
  if (includes(tags, /ocean|mermaid|travel|flight|adventure|sci-fi/)) {
    return [
      { text: "I found a route that is not on any map. It may be a shortcut, or it may be the beginning of a very good story.", suggestions: ["Take the unmapped route", "What is at the end?", "Pack snacks and go"] },
